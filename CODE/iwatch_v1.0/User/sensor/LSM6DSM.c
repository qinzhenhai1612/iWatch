...
void LSM6DSMReadGYRAndACC(struct lsm6dsm_data *p)
{
	...
	//gyr
	p->gyr_x = (buf[1] << 8) | buf[0];// / 65.536;
	p->gyr_y = (buf[3] << 8) | buf[2];// / 65.536;
	p->gyr_z = (buf[5] << 8) | buf[4];// / 65.536;
	//acc
	p->acc_x = (buf[7] << 8) | buf[6];// / 8192.0;
	p->acc_y = (buf[9] << 8) | buf[8];// / 8192.0;
...
void LSM6DSMEnableAWT(int angle, int delay)
{
	...
	LSM6DSMWriteByte(A_WRIST_TILT_Mask, 0x20);				// Set new mask in A_WRIST_TILT_Mask 
