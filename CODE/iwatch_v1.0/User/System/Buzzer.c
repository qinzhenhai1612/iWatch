...
sbit buzzer = P2^6;     //和V1.1设计相同，无需修改

#define BUZZER_FQ 4000     //蜂鸣器的谐振频率为4khz
#define Focs  24000000     //系统频率为24mhz
...
{
	P_SW2 |= 0x80;        //P_SW2为外设端口寄存器2，访问特殊功能寄存器前，需要先将P_SW2(BAH)寄存器的最高位置1
	//设置PWM的频率
	PWMCKS = 0x00;        //PWM计数器时钟为系统时钟的1分频
