...
void ADCInit(void)
{
	ADCCFG = 0x2f;   //  speed[3:0] 设置ADC时钟为系统时钟/2/16/speed
	ADC_CONTR = 0x8f;   //使能 ADC模块，选择第16通道
}
/*
   * @brief   采集模拟通道的ADC转换值
   * @param   ch: 通道号
...
int ADCRead(unsigned char adc_ch)
{
	...
	_nop_();
	_nop_();
	while(!(ADC_CONTR & 0x20));      //查询ADC完成标志   ADC_FLAG ADC转换完成请求中断标志，软件清零
	ADC_CONTR &= ~0x20;              //清完成标志
	res = (ADC_RES <<8 ) | ADC_RES;	//读取ADC结果
	return res;
}
/**
  * @brief  3.3V电源使能
...
/**
  * @brief  调用该函数将系统从掉电模式或者深度掉电模式中恢复过来，不要轻易改动这个函数，否则影响功耗
  * @param  无
  * @retval  无
*/
